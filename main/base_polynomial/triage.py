...
def triage_params(params):
    ...
    #Delay dimension
    parameters['delay_dimension'] = params.get('delay_dimension')
    #Time skip
    parameters['time_skip'] = params.get('time_skip')
    ...
    parameters['max_deg_monomials'] = params.get('max_deg_monomials')
    
    #Determine if there is crossed terms in the basis expansion
    parameters['expansion_crossed_terms'] =  params.get('expansion_crossed_terms', True)
    ...
    if not parameters['expansion_crossed_terms']:
        ...
        L = np.array([int(round(scipy.special.comb(parameters['number_of_vertices'] + d - 1, 
                                                   d, exact = True))) for d in range(parameters['max_deg_monomials'] + 1)])
        
        parameters['L'] = int(round(L.sum()))
    ...
    #Choose the Chebyshev basis functions 
    parameters['use_chebyshev'] =  params.get('use_chebyshev', False)
    ...
    #To use Lebesgue integration with Logistic density
    parameters['use_Logistic'] = params.get('use_logistic', False)
    ...
    #To use all clustered trajectories in a density function
    parameters['single_density'] = params.get('single_density', False)
    parameters['cluster_density'] = params.get('cluster_density', True)
    ...
    if(parameters['use_orthonormal']):
        ...
        # Check if the given params has already calculated the densities.
        if x_t[0] in params:
            for id_node in range(parameters['number_of_vertices']):
                parameters[x_t[id_node]]['lower_bound'] = params.get('lower_bound')
                parameters[x_t[id_node]]['upper_bound'] = params.get('upper_bound')
                            
                parameters[x_t[id_node]]['type_density'] = params.get('type_density')
                parameters[x_t[id_node]]['density'] = params.get('density')
                parameters[x_t[id_node]]['density_normalization'] = params.get('density_normalization')
        
        # Otherwise, it computes from the trajectory using the specified choice of parameters.
        else:
            if not parameters['use_kernel']: 
                
                if parameters['use_Logistic']:
                ...
                if(parameters['use_lebesgue']):
                    ...
                    for id_node in range(parameters['number_of_vertices']):
                        ...
                        parameters[x_t[id_node]]['type_density'] = params.get('type_density', 'leb')
                        parameters[x_t[id_node]]['density'] = params.get('density', lambda x : 1.0)
                        parameters[x_t[id_node]]['density_normalization'] = 1.0
                ...
                if parameters['density'] == None:
                    ...
                    if len(parameters['X_time_series_data'] > 0):
                        ...
                        if parameters['single_density']:        
                            ...
                            single_kernel = stats.gaussian_kde(Opto_orbit, bw_method = 5e-2)
                        ...
                        for id_node in range(parameters['number_of_vertices']):
                            ...
                            if parameters['cluster_density']:
                                num_clusters = len(parameters['cluster_list'])
                                id_vec = np.arange(0, params['number_of_vertices'], dtype = int)
                                for id_cluster in range(num_clusters):
                                    id_vec_cluster =  np.asarray(parameters['cluster_list'][id_cluster], dtype = int)
                            ...
                            if not (parameters['single_density'] and parameters['cluster_density']):
                                ...
                                kernel = stats.gaussian_kde(node_traj, bw_method = 5e-2)
        ...
        if(parameters['use_orthnorm_func_data']):
            ...
            if os.path.isfile(parameters['orthnorm_func_filename']):
                ...
                parameters['orthnormfunc'] = polb.SympyDict.load(parameters['orthnorm_func_filename'])
        ...
        if(parameters['orthnorm_func_map']):
            ...
            parameters['orthnormfunc'], pi_transf = polb.map_power_indices(parameters, temp_power_indices, power_indices)
        
            #To adjust minor differences
            new_power_indices_ = polb.delete_equal_rows(pi_transf, power_indices)
