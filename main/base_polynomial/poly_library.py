...
def map_power_indices_embedding(params, pi_old):
    
    params_orthnormfunc_new = dict()
    power_indices = []
    for l, exp_vec_old in enumerate(pi_old):

        # Create a list of proto exps that have permutations
        proto_exponents = []
        for id_node in range(pi_old.shape[1]):

            # Create a numpy array full of zeros
            proto_exp = np.zeros(params['delay_dimension'])
            # The first element is assigned to be the degree of the exponent
            proto_exp[0] = exp_vec_old[id_node]
        
            # The proto_exp represents the reference exponent of the k delayed coordinates
            # Creates a set of the unique elements by taking permutations
            proto_exponents.append(set(itertools.permutations(proto_exp)))

        
        delay_exp_tuples = list(itertools.product(*proto_exponents))
        delay_exponents = np.array(delay_exp_tuples, dtype = int)
        delay_exponents = delay_exponents.reshape(len(delay_exp_tuples), 
                                                  params['number_of_vertices']*params['delay_dimension'])            
    
        for delay_exp in delay_exponents:
            
            params_orthnormfunc_new[tuple(delay_exp)] = tuple(pi_old[l, :]) #params['orthnormfunc'][tuple(pi_old[l, :])]
            power_indices.append(delay_exp)
    
    return params_orthnormfunc_new, np.array(power_indices, dtype = int)
...
def chebyshev_poly(order):
    '''
    Chebyshev Polynomial function to be used in the moment calculation
    ...
    if order == 1:
        poly_order = lambda x: x
        return poly_order 

    if order > 1:
        poly_order = lambda x: 2*x*chebyshev_poly(order - 1)(x) - chebyshev_poly(order - 2)(x)
        
        return poly_order
    
def chebyshev_polynomial_exp(X_t, exp_vector):
    '''
    ...
    '''
    homo_monomial = chebyshev_poly(exp_vector[0])(X_t[:, 0])

    for id_deg in range(1, len(exp_vector)):
        
        homo_monomial = homo_monomial*chebyshev_poly(exp_vector[id_deg])(X_t[:, id_deg])
...
def spy_gram_schmidt(exp_array, params_, power_indices):
    ...
    d = int(params_['number_of_vertices']/params_['delay_dimension'])
    ...
    try:        #First attempt to access the orthonormal function calculated at
         ...
         if supp.shape[0] == 1:
             for id_ in supp:
                exp_array_mod = np.zeros(np.array(exp_array).shape[0])
                id_mod_d = np.mod(id_, d, dtype = int)
                exp_array_mod[id_mod_d] = exp_array[id_]
             
                params['orthnormfunc'][exp_array] = params['orthnormfunc'][tuple(exp_array_mod)].subs(x_t[id_mod_d], x_t[id_])
            ...
            #identified by exp_array.    
                ...
                if num_vars == 1:
                    ...
                    for id_ in supp:
                        ...
                        id_mod_d = np.mod(id_, d, dtype = int)
                        exp_temp[id_mod_d] = exp_array[id_]
                        
                        func = func*params['orthnormfunc'][tuple(exp_temp)].subs(x_t[id_mod_d], x_t[id_])
...
def reduced_index_iteration(params):
    ...
    if params['expansion_crossed_terms']:            
        ...
        for num_var in range(1, order + 1):
            for deg in range(1, order + 1):     
...
def index_iteration(params):
    ...
    if params['expansion_crossed_terms']:
        ...
        #Monomials functions organized in lexicographic order
        for num_var in range(2, params['max_deg_monomials'] + 1):
...
def reduced_poly_orthonormal_PHI(X_t, params, reduced_pindex):
    ...
    #start_time = time.time()
    for num_var in tqdm(range(1, order + 1),**tqdm_par_red):    
        one_enter = True
...
def library_matrix(X_t, params): 
    ...
    order = params.get('max_deg_monomials')              #order - Maximum degree in the polynimal expansion
    ...
    chebyshev_basis = params.get('use_chebyshev', False)
    ...
    if chebyshev_basis:
        ...
        if not crossed_terms_condition:
            for deg in range(1, order + 1):
                index_vec = np.arange(1, L, order) + (deg - 1)
                PHI[:, index_vec] = chebyshev_poly(deg)(X_t)/np.sqrt(M)
        ...
        if crossed_terms_condition:
            for l in range(power_indices.shape[0]):
                PHI[:, l] = chebyshev_polynomial_exp(X_t, power_indices[l, :])/np.sqrt(M)
    ...
    if(orthonormal_basis):
        ...
        if not crossed_terms_condition:
            ...
            if 'orthnormfunc' in params:
                print('Orthonormal functions have been constructed.')
            else:
